function primeFactors(n){
    ...
    if (n > 2)
        unCountedFactors.push(n)
    unCountedFactors.forEach(function (x) { countedFactors[x] = (countedFactors[x] || 0) + 1; });
...
function formatPrimeFactors(num) {
    ...
    for (const [key, value] of Object.entries(factors)) {
        code = code + `<mult></mult>${key}<sup>${value === 1 ? '' : value}</sup>`
    }
...
const updateTime = () => {
    ...
    dateText.innerHTML = `${dayNames[time.getDay()]}, ${month[time.getMonth()]} ${formatPrimeFactors(time.getDate())}<br><year>${formatPrimeFactors(time.getFullYear())}</year>`
}
