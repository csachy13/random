...
function tick() {
    ...
    timeAfter.innerHTML = `<b>Time after timer:</b> ${padZeros(
        currentTime.getHours()
    )}:${padZeros(currentTime.getMinutes())}:${padZeros(
...
function startTimer() {
    ...
    var seconds =
        minutes * 60 + parseInt(document.getElementById("seconds").value);
    ...
    var timer = setInterval(function () {
        ...
        display.innerHTML =
            ...
            ":" +
            remainingSeconds +
            '<span class="dull white"> left</span>';
        if (seconds <= 0 || !atimer) {
            clearInterval(timer);
